...
Firebase structure:
    ...
    DPMods_Security/
        App_Status/
        Keys/{keyId}
            -> {Username, DeviceLimit, ExpiryDate,
        ...
        Bot_Sessions/{chat_id}
            -> {Key, ExpiryDate, Pending?, LogMessages?}

Features
========
1. /start shows the GalvnicEngine admin-panel referral message.
2. User pastes Referral Key to activate.
3. Generate License Key gives:
      - 🤖 Manual Key
      - ✏️ Name Key
4. Manual Key creates DP-VIP-XXXXXX automatically.
5. Name Key allows a custom license-key name.
6. Duration:
      - 1 Day
      - 3 Days
      - 7 Days
      ...
      - 15 Days
      - 1 Month
7. Device limit is requested after duration.
8. Keys List shows active, expired and banned keys.
9. Device usage is shown as used/limit, e.g. 3/10.
10. /removekey KEY permanently removes a license key.
11. /deletelog msg removes saved Keys List messages.
"""
...
def _firebase_sign_in():
    ...
    _token_cache["expires_at"] = (
        time.time() + int(data["expiresIn"]) - 60
    )
...
def _firebase_refresh():
    ...
    _token_cache["expires_at"] = (
        time.time() + int(data["expires_in"]) - 60
    )
...
def get_used_device_count(key_data):
    ...
    for device_id in devices.keys():
        ...
        # Old schema may contain this placeholder.
        if device_id == "dummy":
            continue
...
# --------------------------------------------------------------------------
# Session / Access Key logic
# --------------------------------------------------------------------------

def get_session(chat_id):
    data = fb_get(
        f"{ROOT}/Bot_Sessions/{chat_id}"
    )
    ...
    key_exp = parse_datetime(
        key_data.get("ExpiryDate")
    )

    if key_exp:
        if key_exp.date() < utc_now().date():
            delete_session(chat_id)
            return None
...
def clear_pending(chat_id):
    fb_delete(
        f"{ROOT}/Bot_Sessions/{chat_id}/Pending"
    )
...
def create_license(
    ...
    payload = {
        ...
        "ExpiryDate": expires_at.strftime(
            "%Y-%m-%d"
        ),
        "ExpiresAt": expires_at.strftime(
            "%Y-%m-%dT%H:%M:%SZ"
        ),
        "Devices": {},
        "Banned": False,
...
def send_keys_list(chat_id):
    ...
    for key_id, data in all_keys:
        ...
        used_devices = (
            get_used_device_count(
                data
            )
        )

        device_limit = data.get(
            "DeviceLimit",
            0,
        )
...
def remove_license_key(
    ...
    send_message(
        ...
        f"🔑 `{key}`\n\n"
        "The license has been deleted from Firebase "
...
def handle_message(message):
    ...
    # ------------------------------------------------------
    # Not activated -> text is Referral Key
    # ------------------------------------------------------

    if not session:

        expire, status = validate_access_key(
            text,
            chat_id,
        )
    ...
    if pending.get(
        ...
        duration_code = pending.get(
            "DurationCode"
        )

        custom_key = pending.get(
            "CustomKey"
        )
...
def webhook():
    ...
    try:
        ...
        print(
            f"[webhook] error handling update: {e}"
        )
...
@app.route(
    f"/register-webhook/{WEBHOOK_SECRET}",
    methods=["GET"],
)
